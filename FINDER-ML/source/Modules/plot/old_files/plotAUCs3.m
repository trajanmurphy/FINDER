...
plotPath = fullfile('..',rF,resultFolder,'Kfold','Graphs2');
...
if ismember(l, ["SVM-Lin", "SVM-RBF", "LogitBoost", "RUSBoost", "BAGging"])
    Balance = "";
end
...
folder = fullfile('..', rF, 'Manual_Hyperparameter_Selection', 'Kfold', 'Graphs2');
...
switch ismember(Best.(Acc).Algo, Benchmarks)
    ...
    case false
        Mres_str.(Acc) = sprintf(', $\\Mres = %d$', Best.AUC.Mres);
end
end
%$\\Mres = %d$
...
% \begin{figure}[H]
% 	\centering
% 	\includegraphics[height = \globalLGheight, width = \globalLGwidth]{Images/SOMAscan7k_KNNimputed_AD_CN.pdf}
% 	\caption{Caption}
% 	\label{CSF_ADCN_LineGraph}
% \end{figure}
...
function X4 = TrimBenchmark(X4)
%ad-hoc function for removing the PCA classifiers
isBenchmark = arrayfun(@(x) x.parameters.multilevel.svmonly == 1, X4);
X5 = X4(isBenchmark);

if isempty(X5), return, end

isPCA = contains(X5.parameters.misc.MachineList, "PCA");

X5.parameters.misc.MachineList = X5.parameters.misc.MachineList(~isPCA);
X5.results.AUC = X5.results.AUC(~isPCA);
X5.results.accuracy = X5.results.accuracy(~isPCA);
X4(isBenchmark) = X5;
