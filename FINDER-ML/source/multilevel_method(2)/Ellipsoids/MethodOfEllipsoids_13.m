function parameters = MethodOfEllipsoids_13(Datas, parameters, methods)
...
[U, ~] = methods.Multi2.svd(Datas.A.CovTraining, parameters, methods);
...
for ima = Truncations
    ...
    %XBima = TransformedB(1:end-ima+1,:);
    Uima = U(:,1:end-ima+1);
    XBima = XB(1:end-ima+1,:);
    [T,~] = methods.Multi2.svd(XBima, parameters, methods);
    %T = svdT(XBima, parameters, methods);
    if strcmp(parameters.multilevel.eigentag, 'smallest'), T = fliplr(T); end
    S = Uima*T;
    %S = ProjectTOntoAMAPerp(T, parameters);
