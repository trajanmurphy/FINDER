...
axValues = {YTicks([1 end]), 'on', 11, 'manual', YTicks, YTickLabels};
...
for iBalance = 1:length(Balances)
        ...
        X(1:2) = [];
        X = {X.name};
        iX = cellfun(@(x) contains(x, '.mat') & ~contains(x, 'Unnormalized'), X) ;
        ...
        xlabeltext = xlabel('$M_{res}$', AxlabelArgs{:}, 'Position', [75, 0.47, 0]); 
...
switch isvm
    case 1 
        xval = [0, Bestx.parameters.data.numofgene];
        [y, iy] = max(Bestx.results.(Acc));
        BestMachine = Bestx.parameters.misc.MachineList(iy);
        y = [y y] ;
        plot(ax, xval, y, 'Color', LineColor, LineArgs{:});
        legstr = [legstr{:}, BestMachine];
...
plotPath = fullfile('..','results','18','Kfold');
exportgraphics(f, fullfile(plotPath, sprintf('ADNI_%s.pdf', Acc)));
