function H = ChebSep2Hessian(K, lambda, Datas, parameters, methods)
...
H = 2*kron(...
    -sub2.MB + ...
    lambda.ineqnonlin(1) * (sub2.CovA -...
     parameters.transform.alpha / 4 * sub2.MB) +...
    lambda.ineqnonlin(2) * (sub2.CovB - ...
    parameters.transform.beta / 4 * sub2.MB),...
    sub2.id(M));
