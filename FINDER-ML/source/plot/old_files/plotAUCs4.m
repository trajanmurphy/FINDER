...
barArray = nan(length(DataSets), 11, 2, 2);
 ...
 for iAcc = 1:length(Accs), Acc = Accs(iAcc);   
    for iAlgo = 1:length(Algos), Algo = Algos(iAlgo);
        ...
        if Algo == "Benchmark"
            ...
            %[parameters, results] = TrimBenchmark(parameters, results);
            barArray(iDS, end-4:end, 1, iAcc) = results.(Acc);
            ...
            for iB = 1:length(Balances), Balance = Balances(iB);
                ...
                barArray(iDS,iBar,iB, iAcc) = arrayfun(@(x) max(x.results.(Acc)), X3);
            end
...
MLScolors = [0.66;1] .* blue;
ACAScolors = t0 .* red ;
ACALcolors = t1 .* red + (1-t1) .* white;
SVMcolors = t0 .* gold;
Othercolors = t2 .* gold + (1-t2) .* white;
...
f = figure('units','normalized','outerposition',[0 0 0.85 0.75]);
nax = 2;
