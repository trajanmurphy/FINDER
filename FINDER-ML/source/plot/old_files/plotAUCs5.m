...
Acc = {"AUC", "accuracy"};
Algo = {"MLS", "ACA-S", "ACA-L", "Benchmark"};
DSidx = [2:5, 8 9 11]; 
...
VT = methods.all.ValuesTable('rF', rF,...
                             'nesting', nesting,...
                             'MOE', MOE,...
                             'Balance', Balances,...
                             'Algo', Algos,...
...
for iBalance = 1:length(Balances)
        ...
        for iAlgo = 1:length(Algos)
            ...
            [x1, y1, l] = GetPlotData(X3, Acc);
            legstr = [legstr, l];


        %% Set up axes
       
        PlotOnAxes(DA, Balance, X4, x1, y1, iAlgo, ax);
...
plotPath = fullfile('..',rF,resultFolder,CrossVal,'Graphs');
...
for iDS = 1:length(DataSets)
    ...
    WriteFigureLatex(fileID, DS, DA, Best);
end
fclose all;
...
for tab = tables
    fileIDs(tables == tab) = fopen(fullfile(folder, ...
        sprintf('%s_%s.txt', tab, nesting)), 'w');
end
...
fprintf(fileID, ['\\centering \n'...
    ...
    'width = \globalLGWidth]'...
    '{Images/%s.pdf} \n'], DS);
fprintf(fileID, ['\\caption{Best AUC: %s (%s %s $\\Mres = %d$) \n' ...
    'Best Accuracy: %s %s $\\Mres = %d$} \n'], ...
    Best.AUC.Performance, Best.AUC.Balance, Best.AUC.Mres,...
    Best.accuracy.Performance, Best.accuracy.Balance, Best.accuracy.Mres);
...
function [x1, y1, l] = GetPlotData(X3, Acc)
...
function PlotOnAxes(DA, Balance, X4, x1, y1, iAlgo, ax)
